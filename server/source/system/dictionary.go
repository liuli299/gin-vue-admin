...
func (d *dictionary) Initialize() error {
	...
	*status = true
	entities := []system.SysDictionary{
		{MODEL: global.MODEL{ID: 1}, Name: "性别", Type: "gender", Status: status, Desc: "性别字典"},
...
func (d *dictionary) CheckDataExist() bool {
	if errors.Is(global.DB.Where("type = ?", "bool").First(&system.SysDictionary{}).Error, gorm.ErrRecordNotFound) { // 判断是否存在数据
		return false
	}
	return true
}
